...
export class PagesService {
    ...
    public getMeterBoxMasters(force: boolean = false, dtrId?): Observable<any> {
        ...
        const PATH = dtrId != undefined ? `${_BASE_URL}/meter-box-masters/${dtrId}` : `${_BASE_URL}/meter-box-masters`;
        return this.httpClient.get(PATH).pipe(
            map(meterBoxMasters => {
    ...
    public getSubstationMasters(force: boolean = false): Observable<any> {
        if (this.substationMasters != null && force != true) {
            return of(this.substationMasters);
        }
        return this.httpClient.get(`${_BASE_URL}/substation-master`).pipe(
            map(substationMasters => {
                this.substationMasters = substationMasters;
                return substationMasters;
            })
        );
    }
    ...
    public convertToDariDate(date) {
        return moment(date, 'YYYY-MM-DD').locale('fa').format('DD-MM-YYYY');
    }

    public convertToGregorianDate(date) {
        return moment.from(date, 'fa', 'DD/MM/YYYY').format('YYYY-MM-DD');
    }
